...
import { QrCode, Calendar, Users, ArrowLeft, CheckCircle2, MapPin, Linkedin } from "lucide-react";
...
const EventPage = () => {
  ...
  const loadAttendees = async (eventId: string) => {
    ...
    if (attendancesData) {
      const attendeesList = attendancesData.map((a: any) => a.profiles).filter(Boolean);
    ...
    <div className="min-h-screen bg-gradient-subtle">
      ...
      <header className="border-b border-border bg-background/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <Link to={currentUserId ? "/dashboard" : "/"} className="flex items-center gap-2">
                ...
                <div className="flex flex-col gap-2 text-muted-foreground">
                  {eventCode && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono font-semibold text-foreground">Event Code: {eventCode}</span>
                    </div>
                  )}
                  {event.starts_at && (
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4" />
                      <span>{new Date(event.starts_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                    </div>
                  )}
                  {event.location && (
                    <div className="flex items-center gap-2">
            ...
            <Card className="shadow-xl">
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-lg font-medium">{attendees.length} Attendees</span>
                </div>
              </CardHeader>
              <CardContent>
                {attendees.length === 0 ? (
                    ...
                    {attendees.map((attendee) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {attendee.headline && (
                            <p className="text-sm text-muted-foreground truncate">{attendee.headline}</p>
                          )}
                        </div>
                        <Button 
                          ...
                          onClick={() => {
                            if (attendee.linkedin_id) {
                              window.open(`https://www.linkedin.com/in/${attendee.linkedin_id}`, '_blank');
                            } else {
